...
void *commands(){
	while(runningFlag){
		...
		if(POLLIN & polls.revents){
			...
			readInfo = SSL_read(client, userInputBuffer, 512);
	...
	while(runningFlag == 1){
		...
		if(elapsedTime >= periodTime){
			...
			if((startFlag == 1) && (runningFlag == 1)){
				...
				SSL_write(client, printBuffer, bufferLength);
			}
			elapsedTime = 0;
		}
	}
...
int main(int argc, char *argv[]){
	...
	//setup SSL (instructions from discussion)
	SSL_library_init();
	...
	if((ctx == NULL) || (client == NULL)){
		fprintf(stderr, "An error occured with SSL.\n");
		exit(1);
	}
	...
	if(connect(wocket, (struct sockaddr*)&server, serverSize) <= -1){
		fprintf(stderr, "An error occured when trying to connect.\n");
		exit(1);
	}

	error2 = SSL_set_fd(client, wocket); 
	error = SSL_connect(client);
	if(error < 1 || error > 1 || error2 == 0){
		fprintf(stderr, "An error occured with the SSL connection.\n");
	}
	
	//ID
	memset(printBuffer, 0, 512);
	sprintf(printBuffer, "ID=%d\n", id);
	SSL_write(client, printBuffer, strlen(printBuffer));
	if(logFlag){
		bufferLength = strlen(printBuffer);
		write(logFile, printBuffer, bufferLength);
	}
	...
	//multithreading
	spoof = sizeof(pthread_t) + sizeof(pthread_t);
	threads = malloc(spoof);
